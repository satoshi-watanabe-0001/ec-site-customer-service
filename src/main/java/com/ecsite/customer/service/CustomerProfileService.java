...
public class CustomerProfileService {
    ...
    @Transactional
    public Customer updateCustomerProfile(UUID customerId, ProfileUpdateRequest request, String currentUserId) {
        ...
        log.info("Customer profile updated successfully - customerId: {}, processingTime: {}ms", 
                customerId, processingTime);

        return savedCustomer;
    }

    private void updateBasicInfo(Customer customer, ProfileUpdateRequest request, String currentUserId) {
    ...
    private void updatePhoneNumbers(Customer customer, ProfileUpdateRequest request) {
        if (request.getPhoneNumbers() != null && !request.getPhoneNumbers().isEmpty()) {
            ...
            for (PhoneNumberUpdateRequest phoneRequest : request.getPhoneNumbers()) {
                CustomerPhoneNumber phoneNumber = CustomerPhoneNumber.builder()
                        .customer(customer)
                        .type(phoneRequest.getType())
                        .number(phoneRequest.getNumber())
                        .isDefault(phoneRequest.getIsDefault() != null ? phoneRequest.getIsDefault() : false)
                        .build();
